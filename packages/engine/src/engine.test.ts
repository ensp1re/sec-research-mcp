...
describe("engine", () => {
  ...
  it("extracts text, renders charts, and rejects unsafe SQL", () => {
    ...
    assert.match(chart.svg, /USD billions|\$/);
    assert.equal(chart.png[0], 0x89);
    assert.equal(formatFact("196502000000", "USD"), "$196.5 billion");
    assert.equal(formatFact("0.500562", "ratio"), "50.1%");
    assert.equal(formatFact(null, "USD"), "missing (not 0)");
